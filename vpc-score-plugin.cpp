...
///////////////////////////////////////////////////////////////////////////////
// Minimal WebSocket server (loopback only, port 3131)
///////////////////////////////////////////////////////////////////////////////
...
std::string base64Encode(const unsigned char* data, size_t len) {
   ...
   while (len--) {
      ...
      if (i == 3) {
         ...
         a4[3] = a3[2] & 0x3f;
         for (i = 0; i < 4; i++) ret += b64chars[a4[i]];
         i = 0;
      }
   }
   if (i) {
      ...
      a4[1] = ((a3[0] & 0x03) << 4) + ((a3[1] & 0xf0) >> 4);
      a4[2] = ((a3[1] & 0x0f) << 2) + ((a3[2] & 0xc0) >> 6);
      for (int j = 0; j < i + 1; j++) ret += b64chars[a4[j]];
      while (i++ < 3) ret += '=';
   }
   return ret;
}
...
void sha1(const std::string& input, unsigned char output[20]) {
   ...
   for (int i=0;i<4;i++){output[i]=(h0>>(24-i*8))&0xFF;output[i+4]=(h1>>(24-i*8))&0xFF;output[i+8]=(h2>>(24-i*8))&0xFF;output[i+12]=(h3>>(24-i*8))&0xFF;output[i+16]=(h4>>(24-i*8))&0xFF;}
}
...
void broadcastWebSocket(const std::string& message) {
   ...
   for (SOCKET client : wsClients) {
      if (client == INVALID_SOCKET) { dead.push_back(client); continue; }
      int flags = 0;
#ifndef _WIN32
      flags = MSG_NOSIGNAL;
#endif
      int result = send(client, reinterpret_cast<const char*>(frame.data()), (int)frame.size(), flags);
      if (result == SOCKET_ERROR || result <= 0) dead.push_back(client);
   }

   for (SOCKET s : dead) {
      auto it = std::find(wsClients.begin(), wsClients.end(), s);
      if (it != wsClients.end()) { closesocket(*it); wsClients.erase(it); }
   }
   if (!dead.empty()) LOGI("Removed %zu disconnected client(s), %zu remain", dead.size(), wsClients.size());
}

// WebSocket server thread — accepts connections and completes handshake
void webSocketServerThread() {
   LOGI("WebSocket server thread starting on 127.0.0.1:3131");

   wsServerSocket = socket(AF_INET, SOCK_STREAM, 0);
   if (wsServerSocket == INVALID_SOCKET) { LOGE("Failed to create server socket"); return; }
   ...
   if (bind(wsServerSocket, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) == SOCKET_ERROR) {
      LOGE("Failed to bind to 127.0.0.1:3131"); closesocket(wsServerSocket); wsServerSocket = INVALID_SOCKET; return;
   }
   if (listen(wsServerSocket, 5) == SOCKET_ERROR) {
      LOGE("Failed to listen"); closesocket(wsServerSocket); wsServerSocket = INVALID_SOCKET; return;
   }

   LOGI("WebSocket server listening on 127.0.0.1:3131 (loopback only)");

   while (wsServerRunning) {
      fd_set readfds; FD_ZERO(&readfds); FD_SET(wsServerSocket, &readfds);
      ...
      if (!FD_ISSET(wsServerSocket, &readfds)) continue;
      ...
      if (total > 0) {
         ...
         if (kp != std::string::npos) {
            ...
            if (ke != std::string::npos) {
               ...
               unsigned char hash[20]; sha1(key, hash);
      ...
      if (!ok) { LOGW("WebSocket handshake failed — closing connection"); closesocket(clientSocket); }
   }
...
#ifdef _WIN32
   HMODULE hm = nullptr;
   if (!GetModuleHandleEx(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS|GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT, (LPCSTR)"VpcScorePluginLoad", &hm)) return "";
   char path[MAX_PATH]; if (!GetModuleFileNameA(hm, path, MAX_PATH)) return "";
   std::string s(path); size_t p = s.find_last_of("\\/"); return (p!=std::string::npos)?s.substr(0,p):"";
#else
   Dl_info info{}; if (!dladdr((void*)&getPluginDirectory, &info) || !info.dli_fname) return "";
   char real[PATH_MAX]; if (!realpath(info.dli_fname, real)) return "";
   std::string s(real); size_t p = s.find_last_of('/'); return (p!=std::string::npos)?s.substr(0,p):"";
#endif
}
...
struct JsonValue {
   ...
   ~JsonValue() { for (auto& p:objValue) delete p.second; for (auto& v:arrayValue) delete v; }

   const JsonValue* get(const char* key) const {
      auto it = objValue.find(key); return (it!=objValue.end())?it->second:nullptr;
   }
...
class SimpleJsonParser {
   ...
   void skipWS() { while(pos<end&&(*pos==' '||*pos=='\t'||*pos=='\n'||*pos=='\r'))pos++; }
   std::string parseString() {
      if(*pos!='"')return""; pos++;
      std::string r;
      while(pos<end&&*pos!='"'){
         if(*pos=='\\'&&pos+1<end){pos++;switch(*pos){case 'n':r+='\n';break;case 't':r+='\t';break;case 'r':r+='\r';break;case '\\':r+='\\';break;case '"':r+='"';break;case 'u':pos+=4;r+='?';break;default:r+=*pos;}}
         else r+=*pos; pos++;
      }
      if(pos<end)pos++; return r;
   }
   int64_t parseNumber() {
      int64_t r=0; bool neg=false; if(*pos=='-'){neg=true;pos++;}
      while(pos<end&&*pos>='0'&&*pos<='9'){r=r*10+(*pos-'0');pos++;}
      if(pos<end&&*pos=='.'){pos++;while(pos<end&&*pos>='0'&&*pos<='9')pos++;}
      return neg?-r:r;
   }
   JsonValue* parseObject() {
      JsonValue* o=new JsonValue(); o->type=JsonValue::OBJECT; pos++; skipWS();
      while(pos<end&&*pos!='}'){skipWS();if(*pos=='}')break;if(*pos!='"'){delete o;return nullptr;}
         std::string k=parseString(); skipWS(); if(pos>=end||*pos!=':'){delete o;return nullptr;} pos++; skipWS();
         JsonValue* v=parseValue(); if(!v){delete o;return nullptr;} o->objValue[k]=v;
         skipWS(); if(pos<end&&*pos==','){pos++;skipWS();}
      }
      if(pos<end)pos++; return o;
   }
   JsonValue* parseArray() {
      JsonValue* a=new JsonValue(); a->type=JsonValue::ARRAY; pos++; skipWS();
      while(pos<end&&*pos!=']'){skipWS();if(*pos==']')break;
         JsonValue* v=parseValue(); if(!v){delete a;return nullptr;} a->arrayValue.push_back(v);
         skipWS(); if(pos<end&&*pos==','){pos++;skipWS();}
      }
      if(pos<end)pos++; return a;
   }
   JsonValue* parseValue() {
      skipWS(); if(pos>=end)return nullptr;
      if(*pos=='"'){JsonValue* v=new JsonValue();v->type=JsonValue::STRING;v->strValue=parseString();return v;}
      if(*pos=='{')return parseObject();
      if(*pos=='[')return parseArray();
      if(*pos=='t'||*pos=='f'){JsonValue* v=new JsonValue();v->type=JsonValue::BOOLEAN;v->boolValue=(*pos=='t');pos+=(*pos=='t'?4:5);return v;}
      if(*pos=='n'){JsonValue* v=new JsonValue();v->type=JsonValue::NULL_TYPE;pos+=4;return v;}
      if(*pos=='-'||(*pos>='0'&&*pos<='9')){JsonValue* v=new JsonValue();v->type=JsonValue::NUMBER;v->numValue=parseNumber();return v;}
      return nullptr;
   }
public:
   JsonValue* parse(const std::string& json){pos=json.c_str();end=pos+json.size();skipWS();return parseValue();}
};
...
std::string decodeBCD(const std::vector<uint8_t>& nvram, size_t start, size_t length) {
   if (start + length > nvram.size()) return "ERROR";
   std::stringstream r;
   if (nvramHighNibbleOnly) {
      for (int i=(int)length-1;i>=0;i--) { uint8_t h=(nvram[start+i]>>4)&0x0F; if(h<=9)r<<(char)('0'+h); }
   } else {
      for (size_t i=0;i<length;i++) {
         uint8_t b=nvram[start+i];
         uint8_t h=(b>>4)&0x0F, l=b&0x0F;
         if(h<=9)r<<(char)('0'+h); if(l<=9)r<<(char)('0'+l);
      }
   }
   return r.str();
}
...
std::string decodeBCDFromOffsets(const std::vector<uint8_t>& nvram, const JsonValue* offsetsArray) {
   if (!offsetsArray || offsetsArray->type != JsonValue::ARRAY) return "ERROR";
   std::stringstream r;
   for (size_t i=0;i<offsetsArray->arrayValue.size();i++) {
      size_t off = parseStartAddress(offsetsArray->at(i));
      if (off >= nvram.size()) continue;
      uint8_t b=nvram[off]; uint8_t h=(b>>4)&0x0F, l=b&0x0F;
      if(h<=9)r<<(char)('0'+h); if(l<=9)r<<(char)('0'+l);
   }
   return r.str();
}
...
std::string applyScale(const std::string& score, int scale) {
   if (scale<=1||score.empty()||score=="ERROR"||score=="???") return score;
   int zeros=0; int s=scale; while(s>1&&s%10==0){zeros++;s/=10;}
   if (zeros>0&&s==1) return score+std::string(zeros,'0');
   try { return std::to_string((unsigned long long)std::stoull(score)*scale); } catch(...) { return score; }
}
...
std::string decodeChar(const std::vector<uint8_t>& nvram, size_t start, size_t length, const std::string& charMap="") {
   ...
   for (size_t i=0;i<length;i++) {
      uint8_t b=nvram[start+i];
      if (!charMap.empty()) r+=(b<charMap.length()?charMap[b]:'?');
      else r+=((b>=32&&b<=126)?(char)b:'?');
   }
   return r;
}
...
size_t parseStartAddress(const JsonValue* startVal, bool adjustForBase) {
   ...
   size_t addr=0;
   if (startVal->type==JsonValue::STRING) addr=std::stoul(startVal->strValue,nullptr,16);
   else if (startVal->type==JsonValue::NUMBER) addr=startVal->numValue;
   if (adjustForBase&&addr>=nvramBaseAddress) addr-=nvramBaseAddress;
   return addr;
}

int decodeValue(const std::vector<uint8_t>& nvram, const JsonValue* fieldObj) {
   if (!fieldObj||fieldObj->type!=JsonValue::OBJECT) return 0;
   const JsonValue* startVal=fieldObj->get("start");
   if (startVal) {
      size_t raw=parseStartAddress(startVal,false);
      if (raw<nvramBaseAddress) return 0;
   }
   size_t start=parseStartAddress(startVal);
   if (start>=nvram.size()) return 0;
   ...
   if (maskVal) {
      if (maskVal->type==JsonValue::STRING) mask=std::stoul(maskVal->strValue,nullptr,16);
      else if (maskVal->type==JsonValue::NUMBER) mask=(uint32_t)maskVal->numValue;
   }
   int offset=0;
   const JsonValue* offVal=fieldObj->get("offset");
   if (offVal&&offVal->type==JsonValue::NUMBER) offset=(int)offVal->numValue;
   int val=0;
   if (enc=="bcd") { uint8_t b=nvram[start]&mask; uint8_t h=(b>>4)&0x0F,l=b&0x0F; if(h<=9&&l<=9)val=h*10+l; }
   else if (enc=="int") val=nvram[start]&mask;
   else if (enc=="bool") {
      const JsonValue* inv=fieldObj->get("invert");
      bool invert=(inv&&inv->type==JsonValue::BOOLEAN&&inv->boolValue);
      val=(nvram[start]&mask)?1:0; if(invert)val=!val;
   }
   return val+offset;
}
...
bool loadNvramMap(const std::string& romName) {
   ...
   if (!indexFile.is_open()) { LOGE("Cannot open index.json: %s", indexPath.c_str()); return false; }
   std::stringstream buf; buf << indexFile.rdbuf();
   SimpleJsonParser parser;
   JsonValue* index = parser.parse(buf.str());
   if (!index||index->type!=JsonValue::OBJECT) { delete index; LOGE("Failed to parse index.json"); return false; }
   ...
   if (!mapFile.is_open()) { LOGE("Cannot open map: %s", mapPath.c_str()); return false; }
   std::stringstream mapBuf; mapBuf << mapFile.rdbuf();
   JsonValue* mapRoot = parser.parse(mapBuf.str());
   if (!mapRoot||mapRoot->type!=JsonValue::OBJECT) { delete mapRoot; LOGE("Failed to parse map: %s", mapPath.c_str()); return false; }

   const JsonValue* meta = mapRoot->get("_metadata");
   if (!meta||meta->type!=JsonValue::OBJECT) { delete mapRoot; LOGE("Missing _metadata in map"); return false; }
   const JsonValue* platVal = meta->get("platform");
   if (!platVal||platVal->type!=JsonValue::STRING) { delete mapRoot; LOGE("Missing platform in _metadata"); return false; }
   ...
   if (!platFile.is_open()) { LOGE("Cannot open platform: %s", platPath.c_str()); return false; }
   std::stringstream platBuf; platBuf << platFile.rdbuf();
   JsonValue* platRoot = parser.parse(platBuf.str());
   if (!platRoot||platRoot->type!=JsonValue::OBJECT) { delete platRoot; LOGE("Failed to parse platform JSON"); return false; }
...
std::string extractScore(const std::vector<uint8_t>& nvram, const JsonValue* scoreEntry) {
   if (!scoreEntry||scoreEntry->type!=JsonValue::OBJECT) return "";
   ...
   if (offsetsVal&&offsetsVal->type==JsonValue::ARRAY) {
      if (encVal->strValue=="bcd") score=decodeBCDFromOffsets(nvram,offsetsVal); else score="???";
      ...
      if (encVal->strValue=="bcd") {
         ...
         uint64_t val=0; size_t len=(size_t)lengthVal->numValue;
         if (addr+len<=nvram.size()) { for(size_t j=0;j<len;j++) val=(val<<8)|nvram[addr+j]; score=std::to_string(val); }
         else score="ERROR";
      } else score="???";
   } else return "";
...
GameState readGameState(const std::vector<uint8_t>& nvram, const std::string& mapPath) {
   ...
   if (!file.is_open()) return gs;
   std::stringstream buf; buf << file.rdbuf();
   SimpleJsonParser parser;
   JsonValue* root = parser.parse(buf.str());
   if (!root||root->type!=JsonValue::OBJECT) { delete root; return gs; }
   const JsonValue* gameState = root->get("game_state");
   if (!gameState||gameState->type!=JsonValue::OBJECT) { delete root; return gs; }
   ...
   if (gs.hasGameOverFlag) gs.gameOver = (decodeValue(nvram, gameOverObj) != 0);
      ...
      if (gs.playerCount == 0) {
         ...
         for (size_t i=0;i<scores->arrayValue.size();i++) {
            ...
            if (!s.empty()&&s!="0"&&s!="ERROR"&&s!="???") {
               bool allZero=true; for(char c:s){if(c!='0'){allZero=false;break;}}
               if (!allZero) inferred=(int)i+1;
            }
         }
...
void onPrepareFrame(const unsigned int /*eventId*/, void* /*userData*/, void* /*eventData*/) {
   ...
   if (wsClients.empty()) return; // no listeners — skip disk I/O

   // Read .nv file from disk
   if (!readNvramFromDisk(currentRomName, nvramData)) return;

   GameState gs = readGameState(nvramData, currentMapPath);

   // Game-over detection
   auto hasNonZero = [](const std::vector<std::string>& s) {
      for (auto& sc:s){if(sc.empty()||sc=="ERROR"||sc=="???")continue;for(char c:sc)if(c!='0')return true;} return false;
   };
   ...
   if (firstStateCheck) {
      ...
      if (gs.currentBall==0 && hasNonZero(gs.scores)) lastGameEndScores = gs.scores;
...
MSGPI_EXPORT void MSGPIAPI VpcScorePluginLoad(const uint32_t sessionId, const MsgPluginAPI* api) {
   ...
   if (pluginDir.empty()) { LOGE("Failed to determine plugin directory"); return; }
...
#ifdef _WIN32
   WSADATA wsaData;
   if (WSAStartup(MAKEWORD(2,2), &wsaData) != 0) { LOGE("WSAStartup failed"); return; }
#endif
   ...
   LOGI("VPC Score Plugin loaded — WebSocket server started on 127.0.0.1:3131");
}
...
MSGPI_EXPORT void MSGPIAPI VpcScorePluginUnload() {
   ...
   {
      std::lock_guard<std::mutex> lock(wsClientsMutex);
      for (SOCKET c : wsClients) { shutdown(c, SHUT_RDWR); closesocket(c); }
      wsClients.clear();
   }
